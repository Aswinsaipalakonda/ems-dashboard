<?php
/**
 * Team Lead - Team Attendance Approvals
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verify this attendance belongs to team member
    $attendance = fetchOne("SELECT employee_id FROM attendance WHERE id = ?", "i", [$attendanceId]);
    if ($attendance && in_array($attendance['employee_id'], $teamMemberIds)) {
        if ($postAction === 'approve' || $postAction === 'reject') {
            ...
            executeQuery(
                ...
                "ssii",
                [$status, $remarks, $userId, $attendanceId]
            );
            
            // Notify employee
            $notifTitle = $postAction === 'approve' ? 'Attendance Approved' : 'Attendance Rejected';
            $notifMessage = $postAction === 'approve' 
                ? 'Your attendance has been approved by your Team Lead' 
                : 'Your attendance has been rejected by Team Lead. Reason: ' . ($remarks ?: 'No remarks');
            ...
            logActivity('team_attendance_' . $postAction, 'Attendance ID: ' . $attendanceId);
            $message = 'Attendance ' . $status . ' successfully!';
            $messageType = 'success';
            $action = '';
        }
    } else {
        $message = 'You can only manage your team members attendance!';
        $messageType = 'danger';
    }
}
...
if (!empty($teamMemberIds)) {
    ...
    $whereClause = "WHERE a.employee_id IN ($placeholders)";
    if ($statusFilter) {
...
<div class="main-content">
    <header class="main-header">
        <div class="header-left">
            ...
            <h1 class="page-title">Team Attendance Approvals</h1>
        </div>
    </header>
    ...
    <div class="content-wrapper">
        ...
        <?php elseif ($action === 'view' && $attendanceDetail): ?>
        <!-- View Details -->
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        ...
                        <!-- Photo Verification -->
                        <?php if (!empty($attendanceDetail['check_in_photo'])): ?>
                        <div class="mb-4">
